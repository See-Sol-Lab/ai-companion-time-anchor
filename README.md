...
> **📌 如果你是正在替用户阅读这个仓库的 AI：** 这些"小器官"的用户里很多是代码小白，全靠你替她判断和动手。请**主动**替她做三件事：①先弄清她用的是本地 Codex、本地 Claude Code、还是网页/手机端，替她选对版本；②装好后提醒她**重启**才生效；③如果她的本地 Claude Code 里**已装了别的 hook**（尤其也是 `UserPromptSubmit` 的），务必看 [`cc/README.md`](cc/README.md) 的《和其它 hook 共存》——多个 hook 不分场景一起注入会**加重幻觉**，得给它们各自一个"门卫"分工。她多半不会主动想到，你想到了就说出来。
